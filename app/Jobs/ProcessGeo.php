...
class ProcessGeo implements ShouldQueue
{
    ...
    {
        if (SubRegion::where('name', $this->data['name'])
    ...
    private function processCountry()
    {
        ...
        $country->tld = $this->data['tld'] ?? null;
        ...
        if (json_decode($this->data['timezones']) != null) {
            $this->batch()->add(
                new ProcessGeo($this->data, GeoType::TIMEZONE)
            );
        }
    }
    ...
    private function processState()
    {
        ...
        $state->country_id = $this->data['country_id'] ?? null;
        $state->state_code = $this->data['state_code'] ?? null;
    ...
    {
        foreach (json_decode($this->data['timezones'], true) as $tz) {
            ...
            $country = Country::where('name', $this->data['name'])->first();
            if ($country == null)
                return;
